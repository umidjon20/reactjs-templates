...
import {NAVBAR_LINKS} from '../Navigation'
export default function Navbar() {
  ...
  return (
      ...
      //   <header className='w-full flex flex-col px-2'>
      //     <nav className='flex items-center justify-between px-4 shadow-md rounded-2xl '>
      ...
      //           }
      //       </ul>
      //       <button className='nav-toggler hidden '>
      //         <span>
  ...
  <div className="w-full text-gray-700 bg-white dark-mode:text-gray-200 dark-mode:bg-gray-800">
    <div 
    // x-data={!open}
     className="flex flex-col max-w-screen-xl px-4 mx-auto md:relative md:items-center  md:justify-between md:flex-row md:px-6 lg:px-8">
      <div className="flex flex-row items-center justify-between p-4">
        <a href="#"
         className="text-lg font-semibold tracking-widest text-gray-900 uppercase rounded-lg dark-mode:text-white focus:outline-none focus:shadow-outline">Flowtrail UI</a>
        <button 
        ...
        onClick={()=>{setOpenNav(!openNav)}}>
          ...
          className="w-6 h-6">
            ...
            (<path
              ...
              ( <path 
                fill-rule="evenodd" d="M3 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM9 15a1 1 0 011-1h6a1 1 0 110 2h-6a1 1 0 01-1-1z" clip-rule="evenodd">
                </path>)}
          </svg>
        </button>
      </div>
      <nav className={ `${openNav ?'flex absolute shadow-md z-40 top-[60px] bg-slate-600 w-screen h-screen':'hidden'} 
      items-center flex-col flex-grow  pb-4 md:pb-0 md:flex md:justify-end md:flex-row`}>
            <ul className={`flex items-center ${openNav ? 'flex-col ':''} gap-5 py-3`}>
                 {
                   NAVBAR_LINKS.map((item)=>(
                     <NavbarLink key={item.key}
                      item={item}/>
                  ))
                }
             </ul>
